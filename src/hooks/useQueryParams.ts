...
export const serialize = (filters: Filter[]): string => {
...
export const deserialize = (str: string): Filter[] => {
  ...
  if (CONSTANTS.LOGICAL_GROUP_REGEX.test(input)) {
    ...
    return [{ key, operator: operator as Operator, type: type as Type, value: value.includes(",") ? value.split(",").map(v => v.trim()) : value.trim() }];
